...
/**
 * ImageDescriptionReverse — Two-column layout with image on right, text on left (reversed)
...
interface ImageDescriptionReverseParams {
  [key: string]: string;
}

export interface ImageDescriptionReverseFields {
  Heading?: TextField;
...
export interface ImageDescriptionReverseProps extends ComponentProps {
  params: ImageDescriptionReverseParams;
  fields: ImageDescriptionReverseFields;
  isPageEditing?: boolean;
}

// ─── Default Variant ────────────────────────────────────────────────────────────

const ImageDescriptionReverseDefault = (
  props: ImageDescriptionReverseProps & { isPageEditing?: boolean }
): JSX.Element => {
  ...
  if (!fields) {
    return (
      <section className="component image-description-reverse" id={id}>
        <div className="component-content">
          <span className="is-empty-hint">ImageDescriptionReverse</span>
        </div>
      </section>
    );
  }
  ...
  return (
    <section data-component="ImageDescriptionReverse" id={id ? id : undefined} className="image-description-reverse-section w-full bg-white py-[40px] md:py-[60px] lg:py-[80px] font-['ADNOC_Sans',sans-serif]">
      <div className="w-full max-w-[1400px] mx-auto px-[16px] md:px-[24px] lg:px-[8px]">
        <div className="flex flex-col lg:flex-row-reverse gap-[24px] lg:gap-[40px] items-center">
...
export const Default: React.FC<ImageDescriptionReverseProps> = (props) => {
  const { page } = useSitecore();
  const isEditing = page?.mode?.isEditing ?? false;
  return <ImageDescriptionReverseDefault {...props} isPageEditing={isEditing} />;
};
